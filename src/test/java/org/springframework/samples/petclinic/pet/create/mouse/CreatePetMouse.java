...
public class CreatePetMouse {
  ...
  private void nextTest(){
  	//Salva il percorso delle pagine in cont e termina il test corrente
  	List<String> pagesOfThisTest=null;
  	if(driver instanceof ChromeDriverPage){
  		pagesOfThisTest=new ArrayList<>(((ChromeDriverPage)driver).htmlPages);
  		((ChromeDriverPage) driver).htmlPages.clear();
	}
  	else if(driver instanceof FirefoxDriverPage){
  		pagesOfThisTest=new ArrayList<>(((FirefoxDriverPage)driver).htmlPages);
		((FirefoxDriverPage) driver).htmlPages.clear();
	}
  	//cont avrà tutte le liste delle pagine
  	cont.add(pagesOfThisTest);
  	//chiudiamo la finestra del caso di test, inizia il prossimo
  }
}
*/
